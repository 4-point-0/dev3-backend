...
export const getNearPublicKey = async (
  username: string,
  ...
  const valid_signature = nacl.sign.detached.verify(msg, signature, publicKey);

  if (!valid_signature) return null;

  return publicKey;
};

export const nearValidatePublicKeyByAccountId = async (
  accountId: string,
  pkArray: string | Uint8Array,
  environment: string,
): Promise<boolean> => {
  const currentPublicKey = edsa + borsh.baseEncode(pkArray);
  const { url, payload, config } = getRpcPostArguments(accountId, environment);

  const result = await axios.post(url, payload, config);
  const value: any = await firstValueFrom(result.data);
